...
export function StructureHeader() {
  ...
  const getRouteLabel = (path: string): string => {
    ...
    return key ? t(key) : t("structure.header.breadcrumb");
  };
  ...
  const getBreadcrumbs = () => {
    ...
    const breadcrumbs = [
      { label: t("structure.header.breadcrumb"), href: "/structure" },
    ];

    if (parts.length > 1) {
      const currentPath = "/" + parts.join("/");
      const label = getRouteLabel(currentPath);
      if (label && currentPath !== "/structure") {
      ...
      <div className="flex items-center gap-3">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-56" align="end" forceMount>
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => navigate("/structure/profil")}>
              <User className="mr-2 h-4 w-4" />
              <span>{t("header.userMenu.profile")}</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => navigate("/structure/settings")}>
