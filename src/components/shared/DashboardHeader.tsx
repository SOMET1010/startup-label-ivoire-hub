...
interface DashboardHeaderProps {
  ...
  rootBreadcrumb: { labelKey: string; href: string };
  profilePath: string;
  settingsPath: string;
...
export function DashboardHeader({
  ...
  const getRouteLabel = (path: string): string => {
    const key = routeKeys[path];
    return key ? t(key) : t(rootBreadcrumb.labelKey);
  };
  ...
  const getBreadcrumbs = () => {
    ...
    const breadcrumbs = [{ label: t(rootBreadcrumb.labelKey), href: rootBreadcrumb.href }];
    if (parts.length > 1) {
      const currentPath = "/" + parts.join("/");
      const label = getRouteLabel(currentPath);
      if (label && currentPath !== rootBreadcrumb.href) {
