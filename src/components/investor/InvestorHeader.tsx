...
export function InvestorHeader() {
  ...
  const getRouteLabel = (path: string): string => {
    ...
    return key ? t(key) : t("investor.header.breadcrumb");
  };
  ...
  const getBreadcrumbs = () => {
    ...
    const breadcrumbs = [
      { label: t("investor.header.breadcrumb"), href: "/investor" },
    ];

    if (parts.length > 1) {
      const currentPath = "/" + parts.join("/");
      const label = getRouteLabel(currentPath);
      if (label && currentPath !== "/investor") {
      ...
      <div className="flex items-center gap-3">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-56" align="end" forceMount>
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => navigate("/investor/profil")}>
              <User className="mr-2 h-4 w-4" />
              <span>{t("header.userMenu.profile")}</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => navigate("/investor/settings")}>
